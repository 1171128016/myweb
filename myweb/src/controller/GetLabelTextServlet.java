...
public class GetLabelTextServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	static Logger logger = Logger.getLogger(GetLabelTextServlet.class);
	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public GetLabelTextServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.debug("获取参数页面为空");
		logger.debug("初始化TextDao类,并且获取全文链接!");
		TextDao text = new TextDao();
		String sql = "SELECT title,texts,time,clickup,browser,comment FROM text ";
		String[] str = {};
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String jsonData = readAsText(streamIn,"UTF-8");
		JSONObject userdata = new JSONObject(jsonData);
		String label = userdata.getString("label");
		logger.debug("获取参数----》"+label);
		logger.debug("初始化TextDao类,获取参数指定数据!");
		TextDao text = new TextDao();
		String sql = "SELECT title,texts,time,clickup,browser,comment FROM text WHERE label=?";
		String[] str = {label};
		String result = text.getList(sql,str);
		logger.debug("查询的文章信息---》"+result);
		out.write(result);
