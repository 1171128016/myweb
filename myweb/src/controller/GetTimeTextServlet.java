...
/**
 * Servlet implementation class GetTimeTextServlet
 */
@WebServlet("/GetTimeTextServlet")
public class GetTimeTextServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	static Logger logger = Logger.getLogger(GetTimeTextServlet.class);
	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public GetTimeTextServlet() {
		super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.debug("获取参数----》"+time);
		logger.debug("初始化TextDao类,获取参数指定数据!");
		TextDao text = new TextDao();
		String sql = "SELECT title,texts,time,clickup,browser,comment FROM text WHERE time=?";
		String[] str = {time};
